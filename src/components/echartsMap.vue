<template>
 ...
 const typedCityJson = cityjson as CityJson;
 ...
 // 直辖市列�?
 const municipalities = ['北京�?, '天津�?, '上海�?, '重庆�?, '香港特别行政�?, '澳门特别行政�?]
 ...
 const getJSON = async (adcode: string) => {
    try {
       ...
       console.warn(`未找到文�? ${filePath}`);
       return null;
    } catch (error) {
       console.error('导入JSON文件时出�?', error);
       return null;
    }
 }
 const initMap = () => {
       ...
       series: [
          {
             ...
             animationDelay: function (idx: number) {
                return idx * 200; // 每个点延�?00ms开始动画，创造波浪效�?
             }
          },
          {
             type: 'map',
